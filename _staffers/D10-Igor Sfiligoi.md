...
title: "Fusion Data Science: Databases and AI Models for Gyrokinetic Transport"
...
summer: |
  ...
  Students should review regression, classification, cross-validation, error metrics, uncertainty estimates, and reproducible data workflows. For domain background, students should read introductory material on magnetic-confinement fusion, tokamak transport, plasma turbulence, and the role of gyrokinetic simulations as expensive physics-based calculations that can be used to train or validate faster reduced models.

  Additional helpful topics include Gaussian-process regression, dimensional analysis, principal-component analysis, uncertainty quantification, and high-performance scientific computing.
...
style: |
  We plan to take an engaged but student-led approach to mentoring. We will work closely with the students throughout the project - meeting regularly, providing guidance, and being available for feedback and support. That said, we're looking for students who are excited to work independently as much as possible.
...
goal: |
  The 1st quarter will be mostly focused on understanding the semantics of the data and ways to extract information out of the raw files. As the final project, we thus plan to have the students create a visualization dashboard or notebook for exploring parameter scans, transport fluxes, growth rates, spectra, and model-validation metrics. That said, if we were to attract students with pre-existing fusion science knowledge, we may pivot into reproducing an existing reduced model study.
---
